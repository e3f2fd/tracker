...
const lastKnownReleases: Map<string, Set<string>> = new Map();
...
app.post('/follow', (req: express.Request<{ userId: string; actorId: string }>, res: express.Response<{ error?: string; ok: boolean }>) => {
	...
	if (!userId || !actorId) {
		res.status(400).json({ error: 'userId and actorId required', ok: false });
		return;
	}
...
async function checkForUpdates() {
	...
	for (const [actorId, userIds] of follows.entries()) {
		try {
			...
			console.error('Error checking actor', actorId, err instanceof Error ? err.message : String(err));
		}
	}
}
